...
function handler(req, res) { // "handler", ki je uporabljen pri require("http").createServer(handler)
    fs.readFile(__dirname + "/Naloga1.html", // povemo, da bomo ob zahtevi ("request") posredovali
...
socket.on("ukazArduinu", function(štUkaza) {
        ...
        if (štUkaza == "7") {
            board.digitalWrite(7, board.HIGH); // zapišemo +5V na pin 8
        }
        if (štUkaza == "6") {
            board.digitalWrite(7, board.LOW); // zapišemo 0V na pin8
        }
    });
});